...
public class SaveDataServiceImp implements SaveDataServiceInterface {
        ...
        try {
            for (ProductComputationlyDto prodCmp:productCompDtoList)
            {
                if(saveDataDaoImp.isProdCompByIdProduct(prodCmp.getUniqueId_Product())){
                    ...
                    prodCompTempEntity = new ProductComputationlyEntity(prodCmp,allDayEntity);
                    saveDataDaoImp.saveOrUpdateProductComps(prodCompTempEntity);
                }
            }
...
//---------------------------------------------get DATA TO DO-------------------------------------------------------------------------------
    ...
    @Override
    public LinkedList<AllDayDto> getAllDayDtoByNumberPage(int countRecord) {

        int defultNumber = 10;


        if(countRecord != 0) {
            saveDataDaoImp.getAllDayByNumberPage(0,countRecord);
        }
